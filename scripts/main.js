...
function exposeApi() {
    ...
    const packageModule = globalThis.game?.modules?.get?.(MODULE_ID) ?? null;
    ...
    console.log("PF2e Exploration Automation | API initialized.");
...
Hooks.on("createRegionBehavior", async behavior => {
    ...
    try {
        await normalizeBehaviorSource(behavior);
    } catch (error) {
        console.error("PF2e Exploration Automation | Failed to normalize a new RegionBehavior.", {
...
async function initializeReady() {
    ...
    /*
     * Normally the API was exposed during init. Retry here
     * defensively in case game.modules was not ready then.
     */
    if (!exposeApi()) {
        console.error(`PF2e Exploration Automation | Module "${MODULE_ID}" is unavailable after the ready hook.`);
        return;
    }
    ...
    try {
        ...
        console.error("PF2e Exploration Automation | Socket registration failed.", error);
        return;
    }

    if (!isPrimaryGM()) {
        console.log("PF2e Exploration Automation | Player or secondary GM ready.");
        return;
    }
    ...
    try {
        const summary = await migrateWorldBehaviors({ notify: false });
        console.log("PF2e Exploration Automation | Behavior migration finished.", summary);
    } catch (error) {
        console.error("PF2e Exploration Automation | Behavior migration failed.", error);
    }
}

/*
 * Normal startup: module file loads first, API is exposed during
 * init.
 *
 * Late diagnostic import: game.modules already exists, so expose it
 * immediately.
 */
if (globalThis.game?.modules?.get?.(MODULE_ID)) {
    exposeApi();
} else {
    Hooks.once("init", () => {
        if (!exposeApi()) {
            console.warn("PF2e Exploration Automation | API was not available during init; ready will retry.");
        }
    });
}

/*
 * Normal startup waits for ready. A late import after ready
 * initializes immediately.
 */
if (globalThis.game?.ready) {
    void initializeReady();
} else {
    Hooks.once("ready", initializeReady);
}
