...
test("Search configuration: rejects a non-GM user", async () => {
    ...
    await runSearchConfiguration({ region: makeRegion() });
    assert.equal(notifications.error.length, 1);
    assert.match(notifications.error[0], /only a GM can add a Search automation/);
});

test("Search configuration: rejects when no Region is provided", async () => {
    const { notifications } = setupWorld();
    await runSearchConfiguration({});
    assert.match(notifications.error[0], /Region is unavailable/);
});

test("Search configuration: editing an existing Behavior falls back to the Behavior's own Region when none is passed", async () => {
    ...
    await runSearchConfiguration({ existingBehavior: existing });
...
test("Search configuration: editing an existing Behavior pre-fills unspecified fields from its current config", async () => {
    ...
    await runSearchConfiguration({ region, existingBehavior: existing });
...
test("Search configuration: valid submission creates a RegionBehavior wired through the socket dispatcher", async () => {
    ...
    await runSearchConfiguration({ region });
